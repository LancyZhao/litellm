...
const getSubmitBlockedReason = (
  ...
  getKeywordTierRulesError(keywordTierRules, config.custom_tier_set) ??
...
const AddAutoRouterTab: React.FC<AddAutoRouterTabProps> = ({
  ...
  const tierEntries = activeTierEntries(complexityRouterConfig);
  ...
  // One in-flight submission: a second click would race the first into a duplicate /model/new.
  const submitRecommendedRouter = async (name: string) => {
    if (isSubmitting) return;
    setIsSubmitting(true);
    try {
      await submitRecommendedRouterInner(name);
    } finally {
      setIsSubmitting(false);
    }
  };

  const submitRecommendedRouterInner = async (name: string) => {
    ...
    const keywordRulesError = getKeywordTierRulesError(keywordTierRules, customTierSet);
