...
export { getCustomTierRowsError, isBuiltInTierName } from "./custom_tier_set";
...
const ComplexityRouterConfig: React.FC<ComplexityRouterConfigProps> = ({
  ...
  // Sole tier-set writer: reconciles both row-id pointers (fallback re-points, floor clears).
  const commitTierRows = (rows: TierDraft[], fallbackTierId: string, base: ComplexityRouterConfigValue = value) => {
    ...
    onChange({ ...base, custom_tier_set: { tiers: rows, fallback_tier_id } });
  };

  const currentFallbackId = customTierSet?.fallback_tier_id ?? "MEDIUM";

  // Removing a built-in row snapshots its models so Restore returns them, not a stale pool.
  ...
  const exitToBuiltInTiers = () => {
    ...
    onChange({
      ...rest,
      tiers: {
        SIMPLE: modelsFor("SIMPLE"),
        MEDIUM: modelsFor("MEDIUM"),
        COMPLEX: modelsFor("COMPLEX"),
        REASONING: modelsFor("REASONING"),
      },
    });
  };
  ...
  // Keyword rules target a tier by name, unlike the id-based fallback and floor pointers, so a
  // rename has to carry them along or it orphans them.
  const updateTierRow = (id: string, patch: Partial<Omit<TierDraft, "id">>) => {
    ...
    if (renamedTo !== undefined && renamedFrom !== undefined && onKeywordTierRulesChange) {
      onKeywordTierRulesChange(
  ...
  return (
    <div className="w-full max-w-none">
      ...
      <span className="block mb-4 text-xs text-muted-foreground">
        {customTierSet
          ? "Display names are unavailable with an edited tier set: your tier names themselves appear in the dashboard and spend logs."
            ...
            <div className="flex flex-wrap items-center gap-2">
              {editingTiers ? (
                <>
                  ...
                  {editingTiers && (
                    <SimpleTooltip content={(customTierSet && getCustomTierRowsError(customTierSet)) || undefined}>
                      <Button
                        variant="outline"
                        disabled={Boolean(customTierSet && getCustomTierRowsError(customTierSet))}
                  ...
                  {customTierSet && (
                    <SimpleTooltip content="Return to the built-in tier ladder. Stored settings that require tier definitions, such as a custom classification prompt, are dropped from the next save.">
          ...
          },
          {
            ...
            children: (
              <>
                ...
                <span className="block text-xs mb-3 text-muted-foreground">
                  {customTierSet ? (
                    "Locked while a tier set is edited: a saved floor still applies while its tier row exists, and editing it for custom tiers arrives with the tier editor's next iteration."
                  ) : (
                    <>
